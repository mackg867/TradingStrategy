########
# VIX/VXV Ratio Filter for UVXY Parameter Sensetivity Analysis
#########
library(quantmod)
...
ptVec = seq(.52,.66,.005)
rptVec = seq(.25,.35,.01)
triggerVec = seq(.95,1.01,.005)

output = matrix(rep(NA,trials)*5,nrow=trials,ncol=5)
...
for(i in 2:length(close)){
  ...
  block = 0
  #Enter
  if(inTrade == 0 && vix[i-1]/vxv[i-1] < trigger){
...
output[z,1] = (runEquity[length(runEquity)]/runEquity[1])^(1/(length(runEquity)/250))
output[z,2:5] = c(sl,pt,rpt,trigger)
}

x = output[!is.na(output[,1]),1]
#hist(x)
